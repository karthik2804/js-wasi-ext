...
const fs = {
    ...
    readdirSync: (path, options = { encoding: "utf-8", withFileTypes: false, recursive: false }) => {
        let dirs = getDirectories();
        path = pathUtility.resolve(process.cwd() || "/", path);
        let closestMatchingDir = findLongestMatchingDirectory(dirs, path);
        if (!closestMatchingDir) {
...
export default fs;
function findLongestMatchingDirectory(descriptors, filePath) {
...
function readDirectory(directory, options, path = '') {
    let entries = [];
